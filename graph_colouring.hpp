...
std::vector<IntegerT>
RowNaturalOrdering
  (const IntegerT i_LeftVertexCount) {
...
ColumnLargestFirstOrdering
  ...
  for (IntegerT i = 0; i < i_VertexCount; ++i) {
    ...
    for (auto j = m_vi_RightVertices[i]; j < m_vi_RightVertices[i + 1]; ++j) {
      ...
      for (auto k = m_vi_LeftVertices[i_Current]; k < m_vi_LeftVertices[i_Current + 1]; ++k) {
      ...
      for (auto k = m_vi_RightVertices[m_vi_Edges[j]];
        ...
        if (m_vi_LeftVertexColors[m_vi_Edges[k]] == m_vi_LeftVertexColors[i]) {
          std::cout << "Left vertices " << i + 1 << " and " << m_vi_Edges[k] + 1
                    << " (connected by right vectex "
                    << m_vi_Edges[j] + 1 << ") have the same color ("
                    << m_vi_LeftVertexColors[i] << ")" << std::endl;
          return false;
        }
      }
    }
      ...
      for (auto w = m_vi_LeftVertices[myVertex];
        ...
        for (auto x = m_vi_RightVertices[m_vi_Edges[w]];
          ...
          if (m_vi_LeftVertexColors[m_vi_Edges[x]] != NOT_ASSIGNED) {
            // !!! each thread should have its private vi_forbiddenColors[] vector
            // to ensure we don't override each other
...
#pragma omp parallel for default(none) schedule(dynamic) \
        shared(i_NumOfVerticesToBeColored, vi_VerticesToBeColored, vi_verticesNeedNewColor)
    for (IntegerT i = 0; i < i_NumOfVerticesToBeColored; i++) {
      ...
      for (auto w = m_vi_LeftVertices[myVertex];
        ...
        for (auto x = m_vi_RightVertices[m_vi_Edges[w]];
          ...
          if (m_vi_LeftVertexColors[m_vi_Edges[x]] == m_vi_LeftVertexColors[myVertex] &&
...
PartialDistanceTwoColumnColoring
  ...
  IntegerT i_LeftVertexCount = down_by_one(m_vi_LeftVertices.size());
  IntegerT i_RightVertexCount = down_by_one(m_vi_RightVertices.size());
  std::vector<IntegerT> m_vi_RightVertexColors(i_RightVertexCount, NOT_ASSIGNED);
  std::vector<IntegerT> vi_forbiddenColors(i_RightVertexCount, NOT_ASSIGNED);
  //auto m_vi_OrderedVertices = std::move(ColumnNaturalOrdering(i_LeftVertexCount, i_RightVertexCount));
  auto m_vi_OrderedVertices = std::move(ColumnLargestFirstOrdering(m_vi_LeftVertices, m_vi_RightVertices, m_vi_Edges));
...
bool CheckPartialDistanceTwoColumnColoring
  ...
  for (IntegerT i = 0; i < down_by_one(m_vi_RightVertices.size()); i++) {
    for (auto j = m_vi_RightVertices[i]; j < m_vi_RightVertices[i + 1]; j++) {
      ...
      for (auto k = m_vi_LeftVertices[m_vi_Edges[j]]; k < m_vi_LeftVertices[m_vi_Edges[j] + 1]; k++) {
        if (m_vi_Edges[k] == i)
          continue;
...
BuildBPGraphFromCSRFormat(const IntegerT *const ip_RowIndex,
  ...
  for (IntegerT i = 0; i < i_RowCount; i++) {
    for (auto j = ip_RowIndex[i] - i_Start;
         j < ip_RowIndex[i + 1] - i_Start; j++) {
...
CalculateVertexDegrees
  (const std::vector<IntegerT> &m_vi_LeftVertices,
   ...
   const std::vector<IntegerT> &m_vi_Edges) {
    ...
    IntegerT m_i_MinimumLeftVertexDegree = m_vi_LeftVertices[1] - m_vi_LeftVertices[0];
    IntegerT m_i_MaximumLeftVertexDegree = m_vi_LeftVertices[1] - m_vi_LeftVertices[0];
    for (IntegerT i = 0; i < i_LeftVertexCount; i++) {
      IntegerT i_VertexDegree = m_vi_LeftVertices[i + 1] - m_vi_LeftVertices[i];
      m_i_MaximumLeftVertexDegree = std::max(m_i_MaximumLeftVertexDegree, i_VertexDegree);
      m_i_MinimumLeftVertexDegree = std::min(m_i_MinimumLeftVertexDegree, i_VertexDegree);
    }

    IntegerT m_i_MinimumRightVertexDegree = m_vi_RightVertices[1] - m_vi_RightVertices[0];
    IntegerT m_i_MaximumRightVertexDegree = m_vi_RightVertices[1] - m_vi_RightVertices[0];
    for (IntegerT i = 0; i < i_RightVertexCount; i++) {
      IntegerT i_VertexDegree = m_vi_RightVertices[i + 1] - m_vi_RightVertices[i];
      m_i_MaximumRightVertexDegree = std::max(m_i_MaximumRightVertexDegree, i_VertexDegree);
      m_i_MinimumRightVertexDegree = std::min(m_i_MinimumRightVertexDegree, i_VertexDegree);
    }
    return std::make_tuple(m_i_MaximumLeftVertexDegree, m_i_MinimumLeftVertexDegree, i_TotalLeftVertexDegree,
                           m_i_MaximumRightVertexDegree, m_i_MinimumRightVertexDegree, i_TotalRightVertexDegree );
  }
